...
function generateEPUB(projectDir, mode = 'chapter', inputDir = null) {
  ...
  if (mode === 'chapter') {
    ...
    if (fs.existsSync(projectRootFile)) {
      ...
      mdFiles = fs.readdirSync(inputDir).filter(f => f.endsWith('.md') && f !== 'AGENDA.md' && !f.startsWith('.'));
...
function main() {
  ...
  if (fs.existsSync(markdownDir)) {
    ...
    if (fs.existsSync(projectRootFile)) {
      ...
      const mdFiles = fs.readdirSync(projectDir).filter(f => f.endsWith('.md') && f !== 'AGENDA.md' && !f.startsWith('.'));
      if (mdFiles.length === 1) {
