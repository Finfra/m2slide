...
const PANDOC_LAYOUT_RESERVED = new Set(['columns', 'column', 'rows', 'row', 'cards', 'htmlart']);
...
function parseMarkdownFile(filePath, autoLayoutDetect = true, tocPlaceholder = false, hasAgenda = true) {
  ...
  const slideObjects = slides.map((slide, index) => {
    ...
    if (isTable(textForSlide)) {
      ...
      // H1(챕터) + H2(슬라이드) 동시 존재 시 H1은 chapterTitle 추적·head_left로 표시되므로
      // 본문에서 H1만 제거하고 H2는 보존 → html-builder가 .title로 호이스팅하여 단일 제목 표시.
      const tableH1 = extractFirstH1(textForSlide);
      const tableHasSub = tableH1.title && extractFirstHeading(tableH1.body).title;
      const tableTitle = tableHasSub ? '' : tableH1.title;
      const tableBody = tableH1.title ? tableH1.body : textForSlide;
      return {
        content: textForSlide,
        rawMarkdown: tableBody,
        title: tableTitle,
        isTable: true,
