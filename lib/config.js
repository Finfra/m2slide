...
function loadConfig(projectDir, cfg) {
  try {
    ...
    // Issue251: _config.yml 에 theme 미설정 시 슬라이드 소스(AGENDA.md / {Name}.md) frontmatter 의 theme: 를 fallback 으로 반영.
    //   우선순위: _config.yml(applyConfig 가 cfg.themeName 설정) > 슬라이드 frontmatter > 'default'.
    //   loadProjectMeta 는 generate-slides.js 에서 이 시점 이후 호출되므로, theme 해석에 맞추려면 여기서 frontmatter 를 1차 파싱해야 함.
    if (!cfg.themeName && projectDir) {
      try {
...
function loadProjectMeta(projectDir, inputDir, cfg) {
  ...
  try {
    ...
    if (!fmMatch) {
      cfg.projectMeta = versionFromFile ? { version: versionFromFile } : {};
      return;
    }
