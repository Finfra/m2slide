...
function applyConfig(raw, cfg) {
  ...
  if (sop) {
    cfg.slideOuterPadding = sop[1].split('#')[0].trim();
  }
  const sip = raw.match(/^slide_inner_padding:\s*(.+)$/m);
  if (sip) {
    cfg.slideInnerPadding = sip[1].split('#')[0].trim();
  }
...
// Issue63: slide_ratio 문자열을 CSS aspect-ratio 호환 수치로 변환
//   '16:9' → '1.7778', '3:2' → '1.5', 'fill' → 'auto' (비율 무제약 명시),
//   그 외(none/잘못된 값/생략) → '1.7778' (16:9 기본)
function slideRatioNumeric(slideRatio) {
  if (typeof slideRatio === 'string' && slideRatio.toLowerCase() === 'fill') return 'auto';
  if (!slideRatio || typeof slideRatio !== 'string') return '1.7778';
  const m = slideRatio.match(/^(\d+(?:\.\d+)?)\s*:\s*(\d+(?:\.\d+)?)$/);
  if (!m) return '1.7778';
  const a = parseFloat(m[1]);
  const b = parseFloat(m[2]);
  if (!isFinite(a) || !isFinite(b) || b === 0) return '1.7778';
