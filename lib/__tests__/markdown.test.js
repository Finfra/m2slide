...
describe('convertMarkdownToHTML — Issue188 htmlArt 구조 도해', () => {
  ...
  test('4종 타입 모두 변환 (process/cycle/hierarchy/pyramid)', () => {
    for (const t of ['process', 'cycle', 'hierarchy', 'pyramid']) {
  ...
  test('내부 리스트는 중첩 ul/li 로 변환', () => {
    const md = '::: htmlart hierarchy\n* 루트\n  - 자식\n:::';
    const html = convertMarkdownToHTML(md, 'controls');
    assert.match(html, /<div class="m2-htmlart htmlart-hierarchy"[^>]*>\s*<ul>/);
