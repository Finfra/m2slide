...
(function(){
  ...
  // 라인은 phrase text 박스 가장자리에 정확히 닿아 단어를 가리킨다.
  function renderExplain(el){
    ...
    var leafW=340, leafH=88, Rr=380, stubLen=32;
    ...
    var gT=svg.append('g');
    d3.range(M).forEach(function(i){
      var ang=i*step - Math.PI/2;                 // 12시 방향부터 시계방향
      var px=cx+Math.cos(ang)*Rr, py=cy+Math.sin(ang)*Rr;
      var dx=px-cx, dy=py-cy;
      // phrase 박스 4면 중 center 방향(가까운) 면 선택 — 라인 끝이 닿는 점.
      // |dx|*halfH vs |dy|*halfW 비교로 horizontal/vertical 면 결정 (박스 모서리 ratio).
      var horizDom = Math.abs(dx)*halfH >= Math.abs(dy)*halfW;
      var faceX, faceY, stubStartX, stubStartY, textAlign, textAlignCSS;
      if(horizDom){
        // 좌·우 면 → horizontal stub
        var sgnX = dx>=0 ? 1 : -1;
        faceX = px - sgnX*halfW;
        faceY = py;
      ...
      gL.append('path')
        .attr('d','M'+cx+' '+cy+' L'+stubStartX+' '+stubStartY+' L'+faceX+' '+faceY)
      ...
      // text 박스는 (px, py) 중심에 배치 — 라인이 가장자리에 정확히 닿음.
    ...
    // 중앙 명제 (박스 없음, 큰 강조)
    var cW=560, cH=140;
    var centerFs=Math.min(72, Math.round(cW*0.11));
