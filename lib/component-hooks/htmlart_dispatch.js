...
(function(){
  ...
  function renderExplain(el){
    ...
    var leafW=340, leafH=88, Rr=380, stubLen=28;
    var cx=Rr+leafW/2+40, cy=Rr+leafH/2+40, W=cx*2, H=cy*2;
    var step=2*Math.PI/M;
    ...
    d3.range(M).forEach(function(i){
      ...
      var stubX=right? px-stubLen : px+stubLen;
      var nx=cx+Math.cos(ang)*(Rr-40), ny=cy+Math.sin(ang)*(Rr-40);
      gL.append('path')
        .attr('d','M'+cx+' '+cy+' L'+nx+' '+ny+' L'+stubX+' '+py)
      ...
      var foX=right? stubX : stubX-leafW;
      var fo=gT.append('foreignObject')
        .attr('x',foX).attr('y',py-leafH/2)
