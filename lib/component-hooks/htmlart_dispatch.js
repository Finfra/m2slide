...
(function(){
  ...
  function nodeBox(parent, x, y, w, h, title, subs, accentBg){
    ...
    var bg=accentBg?'var(--htmlart-accent,#2b8a9d)':'var(--htmlart-box-bg,rgba(0,0,0,.05))';
    ...
    box.append('xhtml:div')
      .attr('style','font-weight:700;font-size:14px;line-height:1.2')
      .text(title);
    if(subs&&subs.length){
      box.append('xhtml:div')
        .attr('style','font-weight:400;font-size:11.5px;line-height:1.25;opacity:.85;margin-top:2px')
  ...
  function renderCycle(el){
    ...
    var N=items.length, W=640, H=640, cx=W/2, cy=H/2;
    var nodeW=176, nodeH=84;
    var Rr=Math.min(cx,cy)-nodeH/2-12;
    ...
    var svg=d3.select(el).append('svg')
      ...
      .attr('style','width:100%;height:auto;max-height:62vh;display:block;margin:0 auto');
    ...
    d3.range(N).forEach(function(i){
      ...
      gArrows.append('path')
        .attr('d', p.toString())
        .attr('style','fill:none;stroke:var(--htmlart-arrow,rgba(0,0,0,.45));stroke-width:3')
        .attr('marker-end','url(#ha-cyc-arrow)');
    });

    svg.append('text')
      .attr('x',cx).attr('y',cy+19).attr('text-anchor','middle')
      .attr('style','font-size:56px;fill:var(--htmlart-accent,#2b8a9d);opacity:.4')
      .text('\\u21BB');
  ...
  function renderHierarchy(el){
    ...
    var nodeW=128, nodeH=46;
    var tree=d3.tree().nodeSize([nodeW+28, nodeH+50]);
    tree(root);
    ...
    var pad=nodeW/2+22;
    var W=(maxX-minX)+pad*2, H=(maxY-minY)+nodeH+44;
    var offX=-minX+pad, offY=-minY+nodeH/2+22;

    el.innerHTML='';
    var svg=d3.select(el).append('svg')
      .attr('viewBox','0 0 '+W+' '+H)
      .attr('class','ha-hierarchy-svg')
      .attr('style','width:100%;height:auto;max-height:60vh;display:block;margin:0 auto');
  ...
  function renderPyramid(el){
    ...
    var bandH=74, gap=8, padT=18;
    var H=padT*2+N*bandH+(N-1)*gap;
    var pyrX=30, pyrMaxW=400, pyrCx=pyrX+pyrMaxW/2;
    var panelX=pyrX+pyrMaxW+34, panelW=250;
    ...
    d3.range(N).forEach(function(i){
      ...
      gBands.append('text')
        .attr('x',pyrCx).attr('y',y0+bandH/2+5).attr('text-anchor','middle')
        .attr('style','font-size:16px;font-weight:700;fill:var(--htmlart-fg,#fff)')
        .text(items[i].title);
  ...
  function renderProcess(el){
    ...
    var N=items.length;
    var boxW=204, boxH=98, arrowGap=58, padY=16;
    ...
    var svg=d3.select(el).append('svg')
      ...
      .attr('style','width:100%;height:auto;max-height:48vh;display:block;margin:0 auto');
