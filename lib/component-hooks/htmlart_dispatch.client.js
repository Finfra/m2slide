(function(){
  ...
  function nodeBox(parent, x, y, w, h, title, subs, accentBg, subFsOverride){
    ...
    //   회귀 방지 — title/subs 양쪽에 token 길이 기반 width cap + overflow-wrap fallback.
    function longestTokenLen(s){
      if(!s) return 1;
      var toks=String(s).split(/[\s·•\-_/|]+/).filter(Boolean);
      var m=1; for(var i=0;i<toks.length;i++){ if(toks[i].length>m) m=toks[i].length; }
      return m;
    }
    var innerW=Math.max(w-32, 40);                // padding 12*2 + safety 8
    var titleTokLen=longestTokenLen(title);
    var titleWidthCap=Math.floor(innerW/(titleTokLen*0.58));  // 0.58em ≈ 영문 sans avg glyph
    var titleFs=Math.min(Math.round(h*0.30), Math.round(w*0.21), 44, titleWidthCap);
    if(titleFs<10) titleFs=10;
    ...
    if(subs && subs.length){
      var subTokLen=1;
      for(var si=0; si<subs.length; si++){ var L=longestTokenLen(subs[si]); if(L>subTokLen) subTokLen=L; }
      var subWidthCap=Math.floor(innerW/(subTokLen*0.58));
  ...
  function renderWorkflow(el){
    ...
    slots.forEach(function(s, idx){
      ...
      if(s.isPerson){
        ...
        nodeBox(gSlots, x, padY, boxW, boxH, s.item.title, s.item.subs, false);
      }
    });
  }
  ...
  function renderAnnotate(el){
    ...
    function drawLines(){
      ...
      boxes.forEach(function(b){
        if(b.x+b.w/2 < cx) left.push(b); else right.push(b);
      });
      left.sort(function(a,b){ return a.y-b.y; });
      right.sort(function(a,b){ return a.y-b.y; });
  ...
  function renderCallout(el){
    ...
    function pickZones(orient, N){
      ...
      if(orient==='fan'){
        var angs=F_p[N]||F_p[8].concat(d3.range(N-8).map(function(){return 90;}));
        var r=460;
