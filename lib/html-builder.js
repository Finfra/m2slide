...
const { getSubsections, getParentPage, getNextChapter } = require('./agenda');
  ...
  if (layoutName && _cfg.layoutTemplates[layoutName]) {
    ...
    const cardsHtml = (slide.children && slide.children.length > 0)
      ? `<ul class="chapter-list chapter-list--cards" data-count="${slide.children.length}">\n` +
        slide.children.map(c => `<li class="chapter-card"><a href="#/${c.index}">${c.title}</a></li>`).join('\n') +
        '\n</ul>'
      : '';
...
function generateHTML(filePath, agendaPath, outputDir, isFirstFile = false, options = {}) {
  ...
  // Determine Reveal.js config based on _cfg.slideRatio
  // Issue65: 유효값은 '16:9' | '3:2' | 'fill' 3종 (config.js 화이트리스트로 검증됨)
  //   '16:9' → 1920×1080 fixed, '3:2' → 1920×1280 fixed, 'fill' → 100%/100% (비율 무제약)
  let revealWidth = 1920;
  let revealHeight = 1080;
  let ratioClass = 'ratio-16-9';

  if (_cfg.slideRatio === '16:9') {
    revealWidth = 1920;
    revealHeight = 1080;
    ratioClass = 'ratio-16-9';
  } else if (_cfg.slideRatio === '3:2') {
    revealWidth = 1920;
    revealHeight = 1280;
    ratioClass = 'ratio-3-2';
  } else if (_cfg.slideRatio === 'fill') {
    revealWidth = '100%';
    revealHeight = '100%';
    ratioClass = 'ratio-fill';
  }
...
<body class="${_cfg.topAlign ? 'top-align-mode' : ''} ${_cfg.guidLine ? 'guide-line-mode' : ''} media-enlarge-${_cfg.styleConfig.style.theContents.media_container_enlarge || 'original'}" style="
...
function generateCoverHTML({ title, slideCssRel, outputDir }) {
  ...
  // media-enlarge-* 는 적용 제외 (section height:100vh+display:flex 가 layout-cover를 파괴)
  // guide-line-mode / top-align-mode 는 정상 적용
  const coverBodyClasses = [
    _cfg.topAlign ? 'top-align-mode' : '',
    _cfg.guidLine ? 'guide-line-mode' : '',
  ].filter(Boolean).join(' ');
  const bodyClassAttr = coverBodyClasses ? ` class="${coverBodyClasses}"` : '';

  // generateHTML 과 동일한 ratio 클래스 계산 (Issue65: 화이트리스트 외 입력은 config.js에서 차단)
  let ratioClass = 'ratio-16-9';
  if (_cfg.slideRatio === '16:9') ratioClass = 'ratio-16-9';
  else if (_cfg.slideRatio === '3:2') ratioClass = 'ratio-3-2';
  else if (_cfg.slideRatio === 'fill') ratioClass = 'ratio-fill';
...
(function(){
  ...
  document.addEventListener('keydown', function(e){
    if (e.key === 'ArrowUp' || e.keyCode === 38 || e.key === 'ArrowLeft' || e.keyCode === 37) {
      e.preventDefault();
      ${_cfg.coverEnabled ? "window.location.href = 'index.html';" : '// cover_enabled=false: agenda is top-level, no navigation'}
