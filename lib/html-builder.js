...
const { getSubsections, getParentPage, getNextChapter, getPrevChapter, getNextSiblingChapter, getPrevSiblingChapter, getLastChapter, getOutlinePath } = require('./agenda');
...
function generateSlideHTML(slide) {
  ...
  if (layoutName && _cfg.layoutTemplates[layoutName]) {
    ...
    const _hl = _resolveHeadSlot(_cfg.styleConfig.head_left, _cfg.styleConfig.head_right, _outlinePath, HEAD_SEPARATOR);
    const _hr = _resolveHeadSlot(_cfg.styleConfig.head_right, _cfg.styleConfig.head_left, _outlinePath, HEAD_SEPARATOR);
...
function generateHTML(filePath, agendaPath, outputDir, isFirstFile = false, options = {}) {
  ...
  // Issue141: outlinePath 부착 — chapter mode는 AGENDA.md 1회 / single mode는 slide별 H1/H2 fallback
  const baseName = path.basename(filePath, '.md');
  if (hasAgenda) {
    // chapter mode: AGENDA.md outline (deck 내 모든 슬라이드 동일)
    const outlinePath = getOutlinePath(baseName + '.html', agendaPath);
    slides.forEach(s => { s.outlinePath = outlinePath; });
  } else {
    // single mode: slide 내 H1(chapterTitle) → d1, slide의 첫 헤더(H2~H6) → d2 fallback
    // slide.title이 자동 감지로 빈 경우 rawMarkdown에서 첫 헤더 텍스트 추출
    slides.forEach(s => {
      const op = [];
      if (s.chapterTitle) op.push(s.chapterTitle);
      let secondTitle = s.title || '';
      if (!secondTitle) {
        // rawMarkdown 또는 content에서 첫 헤더(H2~H6) 추출
        const src = s.rawMarkdown || s.content || '';
        const m = src.match(/^#{2,6}\s+(.+?)\s*$/m);
        if (m) secondTitle = m[1].trim();
      }
      if (secondTitle && secondTitle !== s.chapterTitle) op.push(secondTitle);
      s.outlinePath = op;
    });
  }
