...
function generateSlideHTML(slide) {
  ...
  if (layoutName && _cfg.layoutTemplates[layoutName]) {
    ...
    // Issue90: contents-body의 첫 자식이 <h2 class="title">이면 contents-body 밖(section 직속 자식)으로 이동
    // (백업본 구조 <section><h2 class="title">+<div class="theContents">와 시각적·구조적 등가).
    // → H2가 .contents-body의 flex 자식이 아니게 되어 .contents-body 박스에 포함되지 않고,
    ...
    if (/^_?contents(_no_title)?$/.test(layoutName)) {
      html = html.replace(
        /<div class="contents-body">(\s*)(<h2[^>]*\bclass="[^"]*\btitle\b[^"]*"[^>]*>[\s\S]*?<\/h2>)([\s\S]*?)<\/div>(\s*<\/section>)/,
        '$2\n  <div class="contents-body">$1$3</div>$4'
      );
    }
...
${BASE_CSS}
  ...
  <style>
    ...
    /* Reveal default: ←/→ right 6.4em/0 bottom 3.2em / ↑ right 3.2em bottom 6.4em / ↓ right 3.2em bottom -1.4em + padding 1.4em */
    /* 좌우 center 거리(6.4em) == 상하 center 거리(6.4em) → default가 정사각 마름모. ↑/↓ override 제거하여 형태 유지 */
    ...
    /* 페이지 번호 — 마름모 정중앙 (화살표 4개 center 평균: .controls 기준 right 5em, bottom 5em — visual 마름모 중심과 일치) */
    .reveal .slide-number {
      position: fixed !important;
      right: calc(var(--r-controls-spacing, 12px) + 5em) !important;
      bottom: calc(var(--r-controls-spacing, 12px) + 5em) !important;
    ...
    /* backward (← 키): 좌측에서 등장 → 우측 모션 (Reveal backward와 동일) */
    ...
    /* cross-page 진입 동안 Reveal.js 내부 트랜지션이 보이지 않도록 .reveal 잠시 숨김 */
...
${slidesHTML}
  ...
  <script>
    ...
    // Issue104: cross-page 진입(?back=1 또는 ?fwd=1) 시 Reveal 내부 트랜지션이 보이지 않도록 .reveal을 잠시 숨김
    try {
      if (location.search.indexOf('back=1') !== -1 || location.search.indexOf('fwd=1') !== -1) {
        document.body.classList.add('m2-cross-loading');
      }
    } catch (e) {}
    ...
    // Issue104: ?back=1 진입 시 forward 트랜지션 억제 + backward 방향 CSS 애니메이션 1회 재생
    Reveal.on('ready', function(){
      try {
        var hasLast = location.search.indexOf('last=1') !== -1;
        var hasBack = location.search.indexOf('back=1') !== -1;
        if (hasLast) {
          var totalH = Reveal.getHorizontalSlides().length;
          if (totalH > 0) {
            if (hasBack) {
              // Backward 진입: 트랜지션 없이 즉시 마지막 슬라이드로 점프 (Reveal 내부 트랜지션 억제)
              Reveal.configure({ transition: 'none' });
              Reveal.slide(totalH - 1, 0);
              // 다음 frame에 .reveal 노출 + backward 애니메이션 시작
              ...
              // Forward 점프 (PgDown 등): 기존 동작 유지
              Reveal.slide(totalH - 1, 0);
            }
          }
        } else if (hasBack) {
          // ?back=1 단독 (Home 등): 첫 슬라이드에서 backward 애니메이션 재생
          requestAnimationFrame(function() {
            document.body.classList.remove('m2-cross-loading');
            document.body.classList.add('m2-back-enter');
            setTimeout(function() {
        ...
        } else if (location.search.indexOf('fwd=1') !== -1) {
          // Issue104: ?fwd=1 — 다음 챕터 forward 진입 시 forward 애니메이션 재생
          requestAnimationFrame(function() {
            document.body.classList.remove('m2-cross-loading');
            document.body.classList.add('m2-fwd-enter');
            setTimeout(function() {
              document.body.classList.remove('m2-fwd-enter');
            }, 450);
          });
        }
      } catch (_) {}
    ...
    document.addEventListener('keydown', function(event) {
      ...
      if (event.key === 'ArrowUp' || event.keyCode === 38) {
        ...
        if (isTocSlide(cur))   { stop(); window.location.href = 'agenda.html'; return; }
      ...
      if (event.key === 'ArrowDown' || event.keyCode === 40) {
        if (isCoverSlide(cur)) { stop(); window.location.href = 'agenda.html'; return; }
        if (isTocSlide(cur)) {
...
document.addEventListener('keydown', function(e) {
  // Issue92_1: ⌘+→ 는 End fallback (no-op)으로 아래에서 처리, 일반 → 만 child 이동
  if (((e.key === 'ArrowRight' || e.keyCode === 39) && !e.metaKey) ||
      e.key === 'ArrowDown' || e.keyCode === 40 ||
      e.key === ' ' || e.keyCode === 32) {
    e.preventDefault(); e.stopImmediatePropagation(); window.location.href = 'agenda.html'; return;
  }
  ...
  // ↑(parent)·⇤(이전 sibling)·⇥(다음 sibling)·← : Cover 최상위·sibling 없음 → 동작 없음
  // Issue92: ',' / '.' 를 Home/End fallback 으로 매핑 — 동일하게 no-op
  // Issue92_1: ⌘+← / ⌘+→ 도 Home/End fallback — 동일하게 no-op
  if (e.key === 'ArrowUp' || e.keyCode === 38 ||
      e.key === 'Home' || e.keyCode === 36 || e.code === 'Comma' ||
      e.key === 'End' || e.keyCode === 35 || e.code === 'Period' ||
...
function generateAgendaHTML({ projectDir, title, tocData, slideCssRel, outputDir, lastChapter = '', mode = 'chapter', coverEnabled = true }) {
  // 다운로드 자산 검출
...
(function(){
  ...
  document.addEventListener('keydown', function(e){
    ...
    if (((e.key === 'ArrowLeft' || e.keyCode === 37) && !e.metaKey) ||
      ...
      e.preventDefault(); e.stopImmediatePropagation();
      if (AGENDA_COVER_ENABLED) window.location.href = 'index.html';
      return;
    }
    ...
    if (((e.key === 'ArrowRight' || e.keyCode === 39) && !e.metaKey) ||
        e.key === 'ArrowDown' || e.keyCode === 40 ||
        e.key === ' ' || e.keyCode === 32) {
      if (nextUrl) { e.preventDefault(); e.stopImmediatePropagation(); window.location.href = nextUrl; }
      return;
    }
    // ⇤ Home / ⇥ End (sibling) → 동작 없음 (Agenda는 메타 페이지, sibling 없음)
    // Issue92: ',' / '.' 를 Home/End fallback 으로 매핑 — 동일하게 no-op
    // Issue92_1: ⌘+← / ⌘+→ 도 Home/End fallback — 동일하게 no-op
