...
function generateHTML(filePath, agendaPath, outputDir, isFirstFile = false, options = {}) {
  ...
  if (hasAgenda) {
    ...
    // single mode (2026-05-10 정책 — numbering 자동 계층 추론, H3 outline 제외):
    //   - H1=d1 (메인 챕터, chapterTitle)
    //   - H2 ancestry: 직전 H2의 numbering(예 "4.2.1")의 부모 numbering("4.2")을 같은 H1 안 H2 list에서 매칭하여 trail에 삽입
    ...
    slides.forEach(s => {
      ...
      if (h2List.length > 0) {
        ...
        if (last.numbering) {
          // numbering ancestor 매칭 — H1 numbering과 일치하는 단계는 skip (H1으로 이미 표시)
