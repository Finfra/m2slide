  ...
  <script>
    ...
    document.addEventListener('keydown', function(event) {
      ...
      if (event.key === 'ArrowUp' || event.keyCode === 38) {
        ...
        // Issue99: 본문 슬라이드 → 같은 deck 직전 anchor (H2 sub-anchor 우선, 없으면 H1 anchor) — 직속 부모 의미 회복
      ...
      if (event.key === 'ArrowDown' || event.keyCode === 40) {
        ...
        // 본문(leaf): Issue99 — Chapter 모드는 다음 챕터로 fall-through, 메시지 없이 즉시 이동
        stop();
        if (M2SLIDE_MODE === 'chapter' && NEXT_CHAPTER && NEXT_CHAPTER !== 'index.html') {
      ...
      if (event.key === 'Home' || event.keyCode === 36 || event.code === 'Comma' || (event.metaKey && event.key === 'ArrowLeft')) {
        ...
        var prevAnchorIdx = findPrevH1AnchorIndex(Reveal.getIndices().h);
        stop();
      ...
      if (event.key === 'End' || event.keyCode === 35 || event.code === 'Period' || (event.metaKey && event.key === 'ArrowRight')) {
        ...
        var nextAnchorIdx = findNextH1AnchorIndex(Reveal.getIndices().h);
        stop();
