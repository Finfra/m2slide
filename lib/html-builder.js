...
function generateHTML(filePath, agendaPath, outputDir, isFirstFile = false, options = {}) {
  ...
  if (hasAgenda) {
    ...
    // single mode (2026-05-10 정책 정정): H1 = d1 (메인 챕터), 직전 H2 = d2 (sub-챕터), H3+ = 슬라이드 제목 (outline 제외)
    // chapterTitle 변경 시 H2 reset (새 H1 메인 챕터 진입 시 sub-챕터 컨텍스트 초기화)
    let lastH1 = null;
    let h2 = '';
    slides.forEach(s => {
      ...
      // 슬라이드 자체에 H2 헤더가 있으면 갱신
      const src = s.content || '';
      const h2m = src.match(/^##\s+(.+?)\s*$/m);
      if (h2m) h2 = h2m[1].trim();
      const op = [];
      if (s.chapterTitle) op.push(s.chapterTitle);
      if (h2 && h2 !== s.chapterTitle) op.push(h2);
      s.outlinePath = op;
    });
  }
