  ...
  <script>
    ...
    function initTocMarkmapIfNeeded() {
      ...
      try {
        ...
        /* fit() 직전 SVG에 logical pixel 크기를 명시 — reveal.js transform: scale의 영향으로
           getBoundingClientRect()가 scaled size를 반환해 markmap이 작게 그리는 문제 회피.
           parentNode.offsetWidth/offsetHeight는 transform 적용 전 logical px이므로 안전. */
        /* refit 핵심:
           markmap.fit()은 svg.getBoundingClientRect()로 컨테이너 크기를 잰다.
           reveal.js의 transform: scale이 부모(.slides)에 걸려 있어 BCR은 scaled(visual) 크기를 반환 →
           윈도우 높이가 작을수록 측정값이 작아져 markmap content도 작게 그려짐 (사용자 분석).
           해결: refit 동안 svg.getBoundingClientRect를 일시 monkey-patch하여
           offsetWidth/offsetHeight(transform 적용 전 logical px)를 반환하도록 함. fit() 후 원복. */
        function refit(){
