...
${slidesHTML}
  ...
  <script>
    ...
    // Issue71+92: 현재 horizontal index 이전에서 가장 가까운 H1 anchor 슬라이드 index 반환
    ...
    // K9+Issue92: 현재 horizontal index 이후에서 가장 가까운 H1 anchor 슬라이드 index 반환 (↓ Single mode)
    function findNextAnchorIndex(currentH) {
      var slides = Reveal.getHorizontalSlides();
    ...
    document.addEventListener('keydown', function(event) {
      ...
      if (event.key === 'ArrowUp' || event.keyCode === 38) {
        ...
        if (isAnchorSlide(cur)){ stop(); gotoTocOrAgenda(); return; }
        // 본문 슬라이드 → 같은 deck 직전 H1 anchor (없으면 모드별 폴백)
        var anchorIdx = findPrevAnchorIndex(Reveal.getIndices().h);
        stop();
      ...
      if (event.key === 'ArrowDown' || event.keyCode === 40) {
        ...
        if (isAnchorSlide(cur)) {
          ...
          stop();
          if (idxD + 1 < Reveal.getHorizontalSlides().length) Reveal.slide(idxD + 1, 0);
          return;
        }
        // 본문(leaf): 동작 없음
        stop();
      ...
      if (event.key === 'Home' || event.keyCode === 36 || event.code === 'Comma') {
        if (isCoverSlide(cur)) { stop(); return; } // Cover: 최상위
        if (M2SLIDE_MODE === 'chapter') {
          stop();
          // 첫 챕터(PREV_CHAPTER 빈값)는 동작 없음
          if (PREV_CHAPTER) window.location.href = PREV_CHAPTER;
          return;
        }
        // Single: 같은 deck 직전 H1 anchor (없으면 동작 없음)
        var prevAnchorIdx = findPrevAnchorIndex(Reveal.getIndices().h);
        stop();
      ...
      if (event.key === 'End' || event.keyCode === 35 || event.code === 'Period') {
        if (M2SLIDE_MODE === 'chapter') {
          stop();
        ...
        // Single 모드: 같은 deck 직후 H1 anchor (없으면 동작 없음)
        var nextAnchorIdx = findNextAnchorIndex(Reveal.getIndices().h);
        stop();
