...
${slidesHTML}
  ...
  <script>
    ...
    // Issue124: 모든 페이지 첫 로드(첫 진입·refresh·새 탭·cross-page 시그널 모두)에서
    // Reveal.js가 hash(#/N) 파싱 후 #/0→#/N으로 이동할 때 설정된 transition을 재생하는 부작용 차단.
    // 항상 transition: 'none'으로 시작 → ready 후 원래 값으로 복원 → 사용자 키 입력은 정상 transition.
    // (Issue123의 cross-page 한정 gating을 일반화)
    Reveal.initialize({
      hash: true,
      ...
      // Issue124: 항상 'none'으로 시작 → ready 후 복원 (아래 Reveal.on('ready') 참조)
      // hash-jump 시 슬라이드 전환 애니메이션 재생 차단. 사용자 키 입력은 ready 후 복원된 값 사용.
      transition: 'none',
    ...
    // Issue124: 'none'으로 시작한 transition을 ready 후 _config.yml 설정값으로 복원.
