...
function generateHTML(filePath, agendaPath, outputDir, isFirstFile = false, options = {}) {
  ...
  if (_cfg.tocPlaceholder && hasAgenda && !options.skipTocPlaceholder) {
    // toc_card_mode: H1 슬라이드 목록을 children으로 수집 (slideRef 방식 — 이후 index 해소)
    let mapChildren;
    if (_cfg.tocCardMode && _cfg.layoutTemplates['_cards']) {
      const h1Children = [];
      for (let i = 0; i < slides.length; i++) {
        const s = slides[i];
        if (!s.isTitle && s.content && /^# /.test(s.content.trimStart())) {
          const m = s.content.trimStart().match(/^# (.+?)(?:\r?\n|$)/);
          if (m) h1Children.push({ title: m[1].trim(), slideRef: s });
        }
      }
      if (h1Children.length > 0) mapChildren = h1Children;
    }
    slides.unshift({
      title: getDocTitle(),
      subtitle: null,
