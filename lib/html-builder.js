...
function generateHTML(filePath, agendaPath, outputDir, isFirstFile = false, options = {}) {
  ...
  if (hasAgenda) {
    ...
    // single mode (2026-05-10 정책 — H3까지 outline 인정):
    //   H1=d1 (메인), 직전 H2=d2 (sub), 직전 H3=d3 (슬라이드 제목)
    //   H1 변경 시 H2/H3 reset, H2 변경 시 H3 reset (헤더 stack 정합)
    let lastH1 = null;
    let h2 = '';
    let h3 = '';
    slides.forEach(s => {
      ...
      if (h2m) { h2 = h2m[1].trim(); h3 = ''; }  // H2 갱신 → H3 reset
      const h3m = src.match(/^###\s+(.+?)\s*$/m);
      if (h3m) h3 = h3m[1].trim();
      const op = [];
      if (s.chapterTitle) op.push(s.chapterTitle);
      if (h2 && h2 !== s.chapterTitle) op.push(h2);
      if (h3 && h3 !== h2 && h3 !== s.chapterTitle) op.push(h3);
      s.outlinePath = op;
    });
  }
