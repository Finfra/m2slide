...
const { convertMarkdownToHTML, setKrokiCacheDir } = require('./markdown');
const { getSubsections, getParentPage, getNextChapter, getPrevChapter, getNextSiblingChapter, getPrevSiblingChapter, getLastChapter } = require('./agenda');
...
// data-background-transition, data-auto-animate (값 없음), data-autoslide
...
function generateSlideHTML(slide) {
  ...
  if (layoutName && _cfg.layoutTemplates[layoutName]) {
    ...
    // Issue137: chapter 모드 markmap 컨테이너 마커 — id="toc-placeholder" 부여
    // (Step 8에서 isTocPlaceholder 플래그가 부여된 슬라이드가 본 경로로 들어옴)
    // findTocSlideSvg / isTocSlide / findTocSlideIndex가 이 id를 참조하여 markmap 렌더 + sibling 점프 인덱스 산출
...
function generateHTML(filePath, agendaPath, outputDir, isFirstFile = false, options = {}) {
  ...
  // Kroki 빌드 타임 캐시: outputDir/kroki/ 에 SVG 저장 → HTML img src로 로컬 참조
  //   서버 endpoint는 _config.yml의 kroki_server: 값으로 override 가능 (기본 https://kroki.io)
  //   캐시 폴더는 slide와 함께 deploy되므로 영구 자산화. 캐시 hit 시 외부 호출 0.
  try {
    ...
    const krokiServer = (_cfg && _cfg.styleConfig && _cfg.styleConfig.krokiServer)
      ...
      || 'https://kroki.io';
    setKrokiCacheDir(krokiDir, 'kroki', krokiServer);
  } catch (e) {
  ...
  // Issue137: isTocPlaceholder 플래그로 markmap 컨테이너 마커 슬라이드 식별 (id="toc-placeholder" 부여 대상).
  // parser autoToc 변환된 첫 슬라이드가 이미 layout-_toc면 그 슬라이드를 마커로 재사용 → 중복 빈 placeholder 슬라이드 회피.
  if (_cfg.layoutTemplates['_toc'] && hasTocItems && !options.skipTocPlaceholder) {
    if (slides.length > 0 && slides[0].isTitle) {
      ...
      slides[0].isTocPlaceholder = true;
    } else if (slides.length > 0 && slides[0].layout === '_toc' && slides[0].autoToc) {
      // Issue137: parser가 H1 + 서브 H2를 autoToc 변환한 첫 슬라이드가 이미 markmap 컨테이너 자격을 가짐
      // → 별도 빈 placeholder를 prepend하지 않고 그 슬라이드를 toc-placeholder 마커로 사용
      slides[0].isTocPlaceholder = true;
    } else {
      // isTitle/autoToc 슬라이드가 없으면 _toc 슬라이드를 맨 앞에 삽입
...
${slidesHTML}
  ...
  <script>
    ...
    function isCoverSlide(slide) {
      ...
      return slide.classList && slide.classList.contains('layout-' + M2SLIDE_COVER_LAYOUT);
    }
    function isTocSlide(slide) {
      if (!slide) return false;
      return slide.id === 'toc-placeholder';
    }
    // Issue71: section anchor — layout-_toc class를 가진 슬라이드 (H1·H2 sub-section autoToc 모두 포함)
    // Issue137: toc-placeholder id를 가진 슬라이드도 anchor 자격 보유.
    //           autoToc 변환으로 markmap 컨테이너와 H1 anchor 역할이 단일 슬라이드에서 합쳐졌기 때문 (중복 빈 슬라이드 제거).
    ...
    document.addEventListener('keydown', function(event) {
      ...
      if (event.key === 'End' || event.keyCode === 35 || event.code === 'Period' || (event.metaKey && event.key === 'ArrowRight')) {
        ...
        // Issue133: sibling 부재 시 agenda.html?fwd=1 fallback (Chapter Issue114 대칭)
        ...
        else window.location.href = 'agenda.html?fwd=1';
        return;
      }
