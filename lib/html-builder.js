...
(function(){
  ...
  // Issue301: Mobile Navigation Arrows
  (function setupMobileNavigation() {
    const prevBtn = document.querySelector('.m2-nav-prev');
    const nextBtn = document.querySelector('.m2-nav-next');

    if (!prevBtn || !nextBtn) return; // No nav arrows in this layout

    prevBtn.addEventListener('click', function(e) {
      e.preventDefault();
      e.stopPropagation();
      if (Reveal.getProgress().v > 0 || Reveal.getProgress().h > 0) {
        Reveal.prev();
      }
    });
    ...
    nextBtn.addEventListener('click', function(e) {
      ...
      const isLastSlide = progress.v + progress.h >= Reveal.getTotalSlides() - 1;

      if (isLastSlide) {
        // Loop to first slide (like keyboard behavior)
