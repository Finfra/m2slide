...
<head>
  ...
  <title>${title}</title>
  <script>
    // Issue110: cross-page 진입(?back=1/?fwd=1) 시 Reveal 초기화 전 raw .reveal flash 차단.
    // body 파싱 전에 documentElement에 클래스 부여 → CSS rule(html.m2-cross-loading .reveal {visibility:hidden}) 즉시 적용
    try {
...
${BASE_CSS}
  ...
  <style>
    ...
    /* Issue110: html(documentElement) 클래스로도 매칭 — body 파싱 전부터 적용되어 raw .reveal flash 차단 */
