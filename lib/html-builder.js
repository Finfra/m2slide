...
${BASE_CSS}
  ...
  <style>
    ...
    ${M2_CROSS_GUARD_CSS}${_crossPageFadeInCss(_cfg)}
  </style>
</head>
...
${slidesHTML}
  ...
  <script>
    ...
    // Issue123: cross-page navigation 시그널(?fwd=1/?back=1/?last=1) 진입 시 Reveal.initialize의
    // transition 옵션을 'none'으로 시작하여 hash jump(#/N) 시 슬라이드 전환 애니메이션 재생 차단.
    // ready 이벤트에서 원래 transition으로 복원 → 이후 사용자 키 조작은 정상 transition.
    var _isCrossPageEntry = (function(){
      var s = window.location.search;
      return s.indexOf('fwd=1') !== -1 || s.indexOf('back=1') !== -1 || s.indexOf('last=1') !== -1;
    })();
    ...
    Reveal.initialize({
      ...
      // Issue123: cross-page 진입 시 transition을 'none'으로 시작 → ready 후 복원 (아래 Reveal.on('ready') 참조)
      transition: _isCrossPageEntry ? 'none' : '${_cfg.animation.defaultTransition}',
      transitionSpeed: '${_cfg.animation.defaultTransitionSpeed}',
      backgroundTransition: _isCrossPageEntry ? 'none' : '${_cfg.animation.defaultBackgroundTransition}',
    ...
    // Issue123: cross-page 진입 시 'none'으로 시작한 transition을 ready 후 원래 값으로 복원.
    // hash jump는 이미 완료된 시점이므로 안전. 이후 사용자 키 입력은 정상 transition 적용.
    Reveal.on('ready', function() {
      if (_isCrossPageEntry) {
        try {
          Reveal.configure({
...
<style>${M2_CROSS_GUARD_CSS}${_crossPageFadeInCss(_cfg)}
</style>
${cssLink}
</head>
