...
${_buildPaletteInlineCss(_cfg)}
  ...
  <style>
    ...
    /* Issue301: ←/→ 도 ↑/↓ 처럼 항상 회색 노출 (reveal 기본은 마지막/첫 슬라이드에서 .enabled 제거 → 숨김).
       챕터 마지막 페이지에서 → 를 숨기지 않고 회색으로 남겨 "다음 챕터로 갈 수 있음"을 표시.
       reveal 의 .enabled 클래스가 붙으면(이동 가능) opacity 1, 아니면 0.25 회색. page-indicator 모드는
       body[data-nav-indicator="page"] 규칙(더 높은 specificity)이 여전히 display:none 으로 숨김. */
    ...
    .reveal .controls .navigate-right {
      ...
      opacity: 0.25;
      transition: opacity 0.2s;
      /* Issue301: reveal 기본은 비활성 ←/→에 translateX(±10px)를 남겨(마름모 밖으로 밀림),
         .enabled 일 때만 transform:none 으로 제자리 복귀시킴. 회색(비활성) 상태에도 항상 노출하므로
         transform:none 을 강제해 활성/비활성 무관하게 마름모 정위치를 유지(우측 → 삐져나옴 방지). */
...
${slidesHTML}
  ...
  <script>
    ...
    function m2UpdateNavControls() {
      ...
      // Issue301: reveal 이 끝단(첫/마지막 슬라이드)에서 ←/→ 버튼에 disabled 속성을 붙이면
      //   disabled 버튼은 click 이벤트를 발화하지 않아 클릭 위임이 무력화된다. 매 slidechanged 마다
      //   disabled 를 제거해 챕터 경계에서도 ←/→ 클릭이 키 네비게이션(다음/이전 챕터)으로 위임되게 한다.
      var leftC = document.querySelector('.reveal .controls .navigate-left');
      var rightC = document.querySelector('.reveal .controls .navigate-right');
      if (leftC) leftC.removeAttribute('disabled');
      if (rightC) rightC.removeAttribute('disabled');
    }
    Reveal.on('ready', function() {
      var upBtn = document.querySelector('.reveal .controls .navigate-up');
      var downBtn = document.querySelector('.reveal .controls .navigate-down');
      ...
      // Issue301: ←/→ 클릭을 키보드 ArrowLeft/ArrowRight 로 위임 → 키 네비게이션 매트릭스 재사용.
      //   마지막 슬라이드 → 는 안내 메시지 후 다음 챕터(?fwd=1), 첫 슬라이드 ← 는 이전 챕터/agenda 로 이동.
      //   capture + stopImmediatePropagation 으로 reveal 기본 click(단순 next/prev, 끝단에서 no-op) 차단.
