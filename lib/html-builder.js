...
function generateSlideHTML(slide) {
  ...
  if (layoutName && _cfg.layoutTemplates[layoutName]) {
    ...
    // Issue92: autoToc(layout-_cards 자동 부여) 슬라이드는 원본 마크다운 헤딩 레벨을 보존
...
function generateHTML(filePath, agendaPath, outputDir, isFirstFile = false, options = {}) {
  ...
  if (_cfg.cardsPlaceholder && _cfg.layoutTemplates['_cards'] && hasTocItems && !options.skipTocPlaceholder) {
  ...
  // Map Slide 처리 (Issue138): markmap svg 표시 슬라이드. layout 없는 단순 `<section id="toc-placeholder">` + svg.
  // toc_placeholder 옵션 활성 시 deck #/0 자리에 prepend (Cards Page보다 먼저).
  if (_cfg.tocPlaceholder && hasTocItems && !options.skipTocPlaceholder) {
    slides.unshift({
      title: getDocTitle(),
      subtitle: null,
      author: null,
...
${slidesHTML}
  ...
  <script>
    ...
    function isAnchorSlide(slide) {
      ...
      return slide.classList.contains('layout-_cards') || slide.classList.contains('layout-_toc');
    }
...
//   ⇥ End : 다음 sibling 없음 → 동작 없음 (Issue139: Issue114 agenda fall-through 제거)
...
document.addEventListener('keydown', function(e) {
  // Issue92_1: ⌘+→ 는 End fallback(Issue139: 무동작)으로 아래에서 처리, 일반 → 만 child 이동
  ...
  // Issue139: ⇥ End — Cover에서 동작 없음 (Issue114 agenda fall-through 제거)
  // Issue92: '.' (Period) fallback / Issue92_1: ⌘+→ fallback 동일 무동작 처리
