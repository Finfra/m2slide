...
function generateHTML(filePath, agendaPath, outputDir, isFirstFile = false, options = {}) {
  ...
  if (hasAgenda) {
    ...
    // single mode: slide 내 H1(chapterTitle)만 챕터로 인정 → d1
    // 정책 (2026-05-10 갱신): outline = 챕터만. slide.title (H2~H6 슬라이드 제목)은 챕터가 아니므로 제외
    // now는 "현재 챕터 제목" 의미 — 슬라이드 제목 무관
    slides.forEach(s => {
      s.outlinePath = s.chapterTitle ? [s.chapterTitle] : [];
    });
  }
