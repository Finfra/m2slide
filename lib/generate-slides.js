...
function main() {
  ...
  if (hasAgenda) {
    ...
    // agenda markmap 확장 보강: AGENDA 에 서브 엔트리(`### [..]`)가 없어 children 이 빈
    // 챕터 노드는 markmap 에서 펼칠 대상이 없어 클릭해도 확장되지 않는다. 그런 챕터에 한해
    // 이미 생성된 챕터 HTML 의 실제 slide section 순서(=ground truth #/N)를 harvest 하여
    // `chapter.html#/N` cross-page 앵커 children 으로 채운다. generateTOCFromFile 의 마크다운
    // 기반 #/N 은 prepend 되는 toc-placeholder Map Slide 를 반영하지 못해 off-by-one 이 나므로
    // 소스가 아닌 산출 HTML 의 DOM 순서를 신뢰한다. 이미 서브챕터가 있는 노드는 건드리지 않아 회귀 없음.
    for (const node of agendaTocData.children || []) {
      if (node.children && node.children.length > 0) continue;
      const hrefM = /href="([^"]+\.html)"/.exec(node.content || '');
      if (!hrefM) continue;
      const htmlFile = hrefM[1];
      const chapterHtmlPath = path.join(outputDir, htmlFile);
      if (!fs.existsSync(chapterHtmlPath)) continue;
      let chapterHtml;
      try { chapterHtml = fs.readFileSync(chapterHtmlPath, 'utf-8'); } catch (_) { continue; }
      // .slides 내부 top-level <section> 순서 = reveal 슬라이드 순서(hashOneBasedIndex → #/1 base).
      // 각 section 의 첫 heading 텍스트를 제목으로 사용. 수직 스택(중첩 section)은 flat 데크에서 미사용.
      const slidesM = /<div class="slides">([\s\S]*)/.exec(chapterHtml);
      const scope = slidesM ? slidesM[1] : chapterHtml;
      const secRe = /<section\b[^>]*>([\s\S]*?)<\/section>/g;
      const items = [];
      let sm, idx = 0;
      while ((sm = secRe.exec(scope)) !== null) {
        idx += 1; // 1-base
        const inner = sm[1];
        const hM = /<h[12][^>]*>([\s\S]*?)<\/h[12]>/.exec(inner);
        let title = hM ? hM[1].replace(/<[^>]+>/g, '').replace(/\s+/g, ' ').trim() : '';
        if (!title) title = `슬라이드 ${idx}`;
        items.push({ content: `<a href="${htmlFile}#/${idx}">${title}</a>`, children: [] });
      }
      if (items.length > 0) node.children = items;
    }
