...
function generateHTML(filePath, agendaPath, outputDir, isFirstFile = false, options = {}) {
  ...
  // Issue58: cover 슬라이드 자동 주입 제거 — cover 내용은 _toc/_agenda layout에서 표시

  // Issue58: children.slideRef → 변이 완료 후 최종 인덱스로 해소
...
(function(){
  ...
  document.addEventListener('keydown', function(e){
    if (e.key === 'ArrowUp' || e.keyCode === 38 || e.key === 'ArrowLeft' || e.keyCode === 37) {
      // Issue58: agenda.html이 최상위 — index.html은 redirect이므로 이동하지 않음
      e.preventDefault();
...
function main() {
  ...
  filesToProcess.forEach((file, idx) => {
    ...
    // Issue58: cover 주입 제거 — isFirstFile 신호 불필요
    const html = generateHTML(inputPath, hasAgenda ? agendaPath : null, outputDir, false);
  ...
  // Issue58: index.html — chapter 모드는 항상 agenda.html redirect, single 모드는 forEach에서 이미 생성됨
  const indexPath = path.join(outputDir, 'index.html');
  if (hasAgenda) {
    console.log('\nWriting index.html as redirect (chapter mode)...');
