...
function parseMarkdownFile(filePath) {
  ...
  slideObjects.forEach(s => {
    ...
    if (h1Match) {
      ...
      if (h2Match) {
        currentH1.children.push({ title: h2Match[1], index: slideObjects.indexOf(s), slideRef: s });
      }
    }
  });
...
function generateHTML(filePath, agendaPath, outputDir, isFirstFile = false, options = {}) {
  ...
  if (LAYOUT_TEMPLATES['_toc'] && hasTocItems && !options.skipTocPlaceholder) {
  ...
  if (isFirstFile && COVER_ENABLED && !hasAgenda && LAYOUT_TEMPLATES['_cover']) {
    // 제목 우선순위: _meta.yml title → MD frontmatter title → 파일명
    let coverTitle = PROJECT_META.title || '';
    if (!coverTitle) {
    ...
    slides.unshift({
      title: coverTitle,
      subtitle: PROJECT_META.subtitle || null,
      author: PROJECT_META.instructor_name || null,
      slogan: null,
      content: '',
      rawMarkdown: '',
...
// .reveal section.layout-cover CSS가 적용되도록 Reveal.js 컨텍스트 필수
function generateCoverHTML({ title, slideCssRel, outputDir }) {
  const coverTitle = title || PROJECT_META.title || '';
  let coverSection;
  if (LAYOUT_TEMPLATES['_cover']) {
    ...
    coverSection = `<section class="layout-cover"><div class="cover-header"><h1 class="cover-title">${coverTitle}</h1></div></section>`;
  }
  ...
  const fontImports = (STYLE_CONFIG.style.global.fontImport || [])
...
<script src="https://cdn.jsdelivr.net/npm/reveal.js@5.0.4/dist/reveal.js"></script>
<script>
Reveal.initialize({ controls: false, progress: false, slideNumber: false, hash: false, center: true, transition: 'none', touch: false });
document.addEventListener('keydown', function(e) {
  if (e.key === 'ArrowRight' || e.keyCode === 39 || e.key === 'ArrowDown' || e.keyCode === 40 ||
      e.key === 'PageDown' || e.keyCode === 34 || e.key === ' ' || e.keyCode === 32) {
    e.preventDefault(); window.location.href = 'agenda.html';
  }
});
document.querySelector('.reveal').addEventListener('click', function() { window.location.href = 'agenda.html'; });
</script>
</body>
</html>`;
}
...
(function(){
  ...
  document.addEventListener('keydown', function(e){
    if (e.key === 'ArrowUp' || e.keyCode === 38 || e.key === 'ArrowLeft' || e.keyCode === 37) {
      e.preventDefault();
      ${COVER_ENABLED ? "window.location.href = 'index.html';" : '// cover_enabled=false: agenda is top-level, no navigation'}
...
function main() {
  ...
  filesToProcess.forEach((file, idx) => {
    ...
    // single 모드의 첫 파일: cover_enabled=true면 #/0 커버 주입 트리거
    // chapter 모드: 항상 false (커버는 별도 index.html 페이지)
  ...
  // index.html — chapter 모드: cover_enabled=true이면 커버 페이지, 아니면 agenda.html redirect
  // single 모드: index.html은 위 forEach에서 이미 생성됨
  const indexPath = path.join(outputDir, 'index.html');
  if (hasAgenda) {
    if (COVER_ENABLED) {
      console.log('\nWriting index.html as cover page (cover_enabled=true)...');
      const coverHTML = generateCoverHTML({ title: agendaTitle, slideCssRel: SLIDE_CSS_REL, outputDir });
      fs.writeFileSync(indexPath, coverHTML, 'utf-8');
