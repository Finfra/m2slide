...
let STYLE_CONFIG = {
  title: { fontSizeScale: 1.0, topPadding: '0' },
  list: { fontSizeMin: '16px', fontSizeMaxRatio: 0.66 }
};
...
function loadConfig() {
  try {
    ...
    // Parse style section
    // Simple regex parsing for nested structure
    ...
    if (styleSection) {
      ...
      lines.forEach(line => {
        ...
        if (indent === 2 && content.endsWith(':')) {
          ...
          const [key, val] = content.split(':').map(s => s.trim());
          if (key && val) {
            if (currentSection === 'title') {
              if (key === 'font_size_scale') STYLE_CONFIG.title.fontSizeScale = parseFloat(val);
              if (key === 'top_padding') STYLE_CONFIG.title.topPadding = val;
            } else if (currentSection === 'list') {
              if (key === 'font_size_min') STYLE_CONFIG.list.fontSizeMin = val;
              if (key === 'font_size_max_ratio') STYLE_CONFIG.list.fontSizeMaxRatio = parseFloat(val);
            }
          }
        }
      });
...
function parseMarkdownFile(filePath) {
  ...
  slideObjects.forEach(s => {
    ...
    if (h1Match) {
      ...
      if (h2Match) {
        currentH1.children.push(h2Match[1]);
      }
    }
  });
...
${slide.content}
  ...
  if (isTitleOnly && slide.children && slide.children.length > 0) {
    const listHtml = '<ul class="chapter-list">\n' +
      slide.children.map(child => `<li>${child}</li>`).join('\n') +
      '\n</ul>';
    html += '\n' + listHtml;
  }
...
<head>
  ...
  <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/highlight.js@11.9.0/styles/github.css">${slideCssLink}
  <style>
...
${slidesHTML}
  ...
  <script>
    ...
    function applyDynamicStyles() {
      ...
      // 1. Apply Title Styles (Global or per-slide?)
      // We apply to all title slides once, or just ensure current one is correct.
      // Applying to all is safer to avoid flickering.
      const titleSlides = document.querySelectorAll('.reveal .slides section.title-slide');
      titleSlides.forEach(slide => {
        const h1 = slide.querySelector('h1');
        if (h1) {
          // Default reveal h1 is usually around 2.5em. We scale relative to that or set absolute?
          // Let's assume base is 2.5em.
          if (config.title.fontSizeScale) {
             // We use !important via style property to override CSS
             h1.style.setProperty('font-size', (2.5 * config.title.fontSizeScale) + 'em', 'important');
          }
          if (config.title.topPadding) {
             h1.style.paddingTop = config.title.topPadding;
          }
        }
      });
      ...
      const lists = currentSlide.querySelectorAll('.theContents ul, .theContents ol');
      if (lists.length === 0) return;

      // We only handle the first list if multiple? Or all?
      // Usually one main list.
      lists.forEach(list => {
        const container = list.closest('.theContents');
        if (!container) return;
        ...
        if (h1) {
          titleSizePx = parseFloat(window.getComputedStyle(h1).fontSize);
           ...
           // Try to find h1 in title slide if this is not one?
           // Or just use config base.
           // If config.title.fontSizeScale is 2.0, base is ~2.5em * 40px base? ~100px.
        }

        const maxFontSize = titleSizePx * (config.list.fontSizeMaxRatio || 0.66);
        const minFontSize = parseFloat(config.list.fontSizeMin || '20px');
        ...
        list.style.fontSize = maxFontSize + 'px';
        list.style.lineHeight = '1.2'; // Tighter line height for large text
        ...
        // Apply best fit
        list.style.fontSize = bestFit + 'px';
        
        // If even at min size it overflows, enable scroll
        if (bestFit <= minFontSize + 1) {
           // Check overflow at min size
           list.style.fontSize = minFontSize + 'px';
           if (list.scrollHeight > container.clientHeight) {
