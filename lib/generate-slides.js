...
function generateAgendaHTML({ projectDir, title, tocData, slideCssRel, outputDir }) {
  ...
  // _toc.html layout 렌더 (시스템 폴백 포함)
  let bodyHtml;
  if (LAYOUT_TEMPLATES['_toc']) {
    bodyHtml = renderLayout('_toc', {
      ...PROJECT_META,
      title: title || '',
      content: '',
      markmap: '',
    ...
    // Fallback (theme에 _toc 없을 때)
    bodyHtml = `<section class="layout-_toc"><header class="toc-page-header"><h1 class="toc-page-title">${title}</h1><div class="toc-page-downloads">${downloadButtons}</div></header><div class="toc-markmap"><svg class="toc-mindmap-svg"></svg></div></section>`;
  }
