...
// Issue27_2: 본문 선두의 빈 헤더(`## ` 단독) 한 개를 제거
  ...
  if (fs.existsSync(dir)) {
    fs.readdirSync(dir).forEach(f => {
      if (f.endsWith('.html')) {
        ...
        LAYOUT_TEMPLATES[name] = fs.readFileSync(path.join(dir, f), 'utf8');
      }
    });
  }
      ...
      fs.readdirSync(defaultDir).forEach(f => {
        if (f.endsWith('.html')) {
          ...
          if (!LAYOUT_TEMPLATES[name]) {
            LAYOUT_TEMPLATES[name] = fs.readFileSync(path.join(defaultDir, f), 'utf8');
          }
        }
      });
    }
...
function parseMarkdownFile(filePath) {
  ...
  const slideObjects = slides.map((slide, index) => {
    ...
    // 우선순위: _blank (image-only) > _contents_no_title (title-empty)
