...
let TOC_PLACEHOLDER = false; // project-level toc_placeholder default
let COVER_ENABLED = false;   // Issue49: project-level cover_enabled (auto-inject _cover slide)
...
let THEME_NAME = null;             // theme 이름
let THEME_DEFAULT_LAYOUT = null;   // 프로젝트 기본 layout
let LAYOUT_TEMPLATES = {};         // { layoutName: htmlTemplateString }
let USE_OPEN_PROPS = false; // Use Open Props CDN
let TITLE_CONTENTS_GAP = 30; // title height의 % 단위 기본값
let AUTO_LAYOUT_DETECT = true; // Issue27_3: image-only/empty-title 자동 layout 감지 ON/OFF
let VIDEO_DEFAULT = 'controls'; // Issue43: ![](*.mp4) 마크다운 비디오 기본 동작 preset
let PROJECT_META = {};          // Issue48: 프로젝트 운영 메타 (_meta.yml — instructor, version, lecture_date 등)
let PROJECT_DOWNLOADS_HTML = ''; // Issue55: 빌드 시점 EPUB/PDF/PPTX 검출 후 layout `{{downloadButtons}}` 변수에 주입할 HTML
let STYLE_CONFIG = {
...
let CONFIG_BASE_DIR = ROOT_DIR; // base dir for slide_css resolution

// Parse and apply a single config raw string onto global state variables.
// Returns true if slide_css was found in this config (so caller can update CONFIG_BASE_DIR).
function applyConfig(raw) {
    ...
    if (tp) {
      ...
      TOC_PLACEHOLDER = (val === 'true' || val === 'yes' || val === '1');
    }
    const ce = raw.match(/^cover_enabled:\s*(.+)$/m);
    if (ce) {
      const val = ce[1].split('#')[0].trim().toLowerCase();
      COVER_ENABLED = (val === 'true' || val === 'yes' || val === '1');
    }
    const tcg = raw.match(/^title_contents_gap:\s*(.+)$/m);
    if (tcg) {
      const val = parseFloat(tcg[1].split('#')[0].trim());
      if (!isNaN(val)) TITLE_CONTENTS_GAP = val;
    }
    const ald = raw.match(/^auto_layout_detect:\s*(.+)$/m);
    if (ald) {
      const val = ald[1].split('#')[0].trim().toLowerCase();
      AUTO_LAYOUT_DETECT = (val === 'true' || val === 'yes' || val === '1');
    }
    // Issue43: video_default preset 파싱
    const vd = raw.match(/^video_default:\s*(.+)$/m);
    if (vd) {
      const val = vd[1].split('#')[0].trim();
      if (VIDEO_DEFAULT_PRESETS.hasOwnProperty(val)) {
        VIDEO_DEFAULT = val;
      } else {
        console.warn(`⚠️ Invalid video_default: '${val}' — falling back to 'controls' (allowed: ${Object.keys(VIDEO_DEFAULT_PRESETS).join(', ')})`);
      }
    }
...
//   1. ROOT_DIR/_config.org.yml  (base defaults)
//   2. ROOT_DIR/_config.yml      (root override, optional)
//   3. projectDir/_config.yml    (project-level override, optional)
function loadConfig(projectDir) {
  try {
    const orgPath = path.join(ROOT_DIR, '_config.org.yml');
    if (fs.existsSync(orgPath)) {
      const hasCss = applyConfig(fs.readFileSync(orgPath, 'utf-8'));
      if (hasCss) CONFIG_BASE_DIR = ROOT_DIR;
    }
    ...
    for (const c of overrides) {
      if (fs.existsSync(c.p)) {
        const hasCss = applyConfig(fs.readFileSync(c.p, 'utf-8'));
        if (hasCss) CONFIG_BASE_DIR = c.base;
      }
    }

    // Step 3: theme → slide_css 자동 해석 + fallback
    // slide_css가 설정되었더라도 실제 파일이 없으면 theme 해석 시도
    const slideCssAbsPath = SLIDE_CSS_REL
      ? (path.isAbsolute(SLIDE_CSS_REL) ? SLIDE_CSS_REL : path.join(CONFIG_BASE_DIR, SLIDE_CSS_REL))
      : null;
    const slideCssExists = slideCssAbsPath && fs.existsSync(slideCssAbsPath);
    if (!SLIDE_CSS_REL || !slideCssExists) {
      const themeName = THEME_NAME || 'default';
      const themeCss = path.join(ROOT_DIR, 'theme', themeName, 'slide.css');
      if (fs.existsSync(themeCss)) {
        SLIDE_CSS_REL = themeCss;
        CONFIG_BASE_DIR = ROOT_DIR;
    ...
    // Step 6: layout 템플릿 로드 (theme 해석 완료 후)
    loadLayoutTemplates(THEME_NAME || 'default');
  } catch (_) { }
}

// Issue48: 프로젝트 운영 메타데이터 옵셔널 로드
// _doc_design/meta-yml.md 스키마 참조. 미존재 시 silent skip (backward compatible). 파일명: _meta.yml
function loadProjectMeta(projectDir) {
  ...
  try {
    ...
    // YAML frontmatter (`---` ... `---`) 또는 본문 단독 YAML 모두 허용
...
// Issue55: 다운로드 자산 검출 + 버튼 HTML 조합 (Phase 2)
// `slide/{ProjectName}.{epub,pdf,pptx}` 존재 여부 검사 후 해당 항목만 <a> 태그 HTML로 조합
// D3=A: 미존재 항목은 출력 문자열에서 완전 제외
...
// === Issue34: Multi-column / Multi-row layout preprocessing ===

// Map raw class name to (m2-* alias) + (original class)
...
function convertMarkdownToHTML(markdown) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.match(/^### /)) {
      ...
      html.push(`<h3>${processInline(content)}</h3>`);
      continue;
    }
    if (line.match(/^## /)) {
      const content = line.replace(/^## /, '');
    ...
    if (imgMatch) {
      ...
      if (isVideoUrl(url)) {
        const attrs = VIDEO_DEFAULT_PRESETS[VIDEO_DEFAULT] || VIDEO_DEFAULT_PRESETS['controls'];
...
// ── Theme/Layout helpers (Steps 4-6) ──────────────────────────────────────────

// extractLayoutMeta: 슬라이드 raw 텍스트에서 #layoutName 메타 추출 (방어적 파서)
// 매칭 규칙: 첫 비어있지 않은 라인이 `#layoutName` 형태여야 함
//   허용: `#contents`, `#_toc`, `#my-layout` (영소문자/숫자/하이픈, _ 1개 prefix)
//   불허: `# contents` (공백), `## sub` (H2), `#가나다` (한글), `#My` (대문자)
function extractLayoutMeta(rawSlideText) {
  const lines = rawSlideText.split('\n');
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    if (line.trim() === '') continue;
    const m = line.match(/^#(_?[a-z][a-z0-9-]*)\s*$/);
    if (m) {
...
// extractFirstH1: rawMarkdown에서 첫 H1을 추출하고 본문에서 제거
...
// Issue27_1: 본문이 단일 이미지로만 구성되어 있는지 감지
// (제목 헤더, 텍스트, 리스트 등이 일체 없고 이미지 1개만 존재)
// Issue43: video 확장자(`.mp4` 등)는 video-only로 분류하고 image-only에서 제외
function isImageOnlySlide(text) {
  const meaningful = text.split('\n')
    .map(l => l.trim())
    .filter(l => l.length > 0 && !l.startsWith('<!--'));
  if (meaningful.length !== 1) return false;
  // 이미지 마크다운: ![alt](url) — alt 비어 있어도 허용
  const m = meaningful[0].match(/^!\[[^\]]*\]\(([^)]+)\)\s*$/);
  if (!m) return false;
  // 비디오 확장자면 image-only가 아님 (isVideoOnlySlide에서 별도 처리)
  return !isVideoUrl(m[1]);
}

// Issue43: 본문이 단일 비디오 임베드(`![alt](*.mp4|.webm|.ogg|.mov|.m4v)`)로만 구성되어 있는지 감지
// 우선순위: isImageOnlySlide와 상호 배타적 (확장자로 분기)
...
// Issue27_2: 슬라이드 제목이 비어 있는지 감지
// 케이스 A: 첫 의미있는 라인이 헤더이지만 텍스트가 비어 있음 (`## ` 단독)
// 케이스 B: 첫 의미있는 라인이 헤더가 아닌 본문 (제목 헤더 자체가 없음)
...
function stripEmptyLeadingHeader(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!stripped) {
      ...
      // 비어있지 않은 첫 콘텐츠 발견 → 더 이상 strip 안 함
      out.push(lines[i]);
      stripped = true;
      continue;
    }
...
// Step 6: layout 템플릿 로드 + 렌더링 함수
// Issue41: underscore prefix 제거 alias도 함께 등록 (`_contents` → `contents` alias)
//   사용자가 `_config.yml`에 `theme_default_layout: contents`로 작성해도 정상 lookup
function _registerLayoutTemplate(name, content) {
  if (!LAYOUT_TEMPLATES[name]) {
    LAYOUT_TEMPLATES[name] = content;
  }
  const stripped = name.replace(/^_/, '');
  if (stripped !== name && !LAYOUT_TEMPLATES[stripped]) {
    LAYOUT_TEMPLATES[stripped] = content;
  }
  // Issue49: 번호 prefix(N.M.) alias — `1.1.cover.html` → `cover` 키도 등록
  // 사용자 테마가 AGENDA 정렬용 숫자 prefix를 쓰는 경우 role-based lookup 가능
  const numStripped = name.replace(/^[\d.]+\./, '');
  if (numStripped !== name && !LAYOUT_TEMPLATES[numStripped]) {
    LAYOUT_TEMPLATES[numStripped] = content;
  }
}

function loadLayoutTemplates(themeName) {
  // Step 1: theme 폴더에서 레이아웃 로드
  ...
  // Step 2: default 테마에서 없는 레이아웃 보충 (theme !== 'default'일 때만)
...
// Issue41: layout 미발견 경고 dedup — 동일 layoutName 빌드당 1회만 출력
const _WARNED_MISSING_LAYOUTS = new Set();

function renderLayout(layoutName, vars) {
  const tpl = LAYOUT_TEMPLATES[layoutName];
  if (!tpl) return null;
...
function parseMarkdownFile(filePath) {
  ...
  const useTocPlaceholder = TOC_PLACEHOLDER || yamlTocPlaceholder;

  const slides = content.split(/\n---\n/).map(slide => slide.trim());

  // Prepend a placeholder slide for the Title/TOC when needed
  if (useTocPlaceholder) {
    slides.unshift('');
  }
  ...
  const slideObjects = slides.map((slide, index) => {
    ...
    // First slide is TOC placeholder when useTocPlaceholder is set
    if (index === 0 && useTocPlaceholder) {
      let title = yamlTitle;
      if (!title) {
        // Derive title from first H1 in remaining slides
    ...
    let autoBody = null;       // Issue27 자동 감지 시 가공된 body
    let autoFullImage = false; // Issue27_1 image-only 마커
    let autoFullVideo = false; // Issue43 video-only 마커
    ...
    // Issue27 자동 layout 감지 (사용자 명시 layout이 없을 때만)
    // Issue27_3: AUTO_LAYOUT_DETECT 플래그로 ON/OFF
    // 우선순위: _blank (image-only / video-only) > _contents_no_title (title-empty)
    if (!layout && AUTO_LAYOUT_DETECT) {
    ...
    // Step 4: layout이 있으면 H1을 title과 rawMarkdown으로 분리 저장
    let slideTitle = '';
    let rawMarkdown = textForSlide;
    if (layout) {
      if (autoBody !== null) {
        // 자동 감지 케이스: title 없음, body는 사전 가공된 텍스트 사용
        slideTitle = '';
...
function getSubsections(fileName, agendaPath) {
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (foundMainSection) {
        // Stop when we hit another main section (visible or hidden)
        if (line.match(/^## !?\[/)) {
          break;
        }

        // Collect subsection (visible only; hidden ### ![ are excluded)
...
function getNextChapter(fileName, agendaPath) {
  try {
    ...
    // Collect all chapter files in order (including hidden ## ![ entries for navigation)
    const chapters = [];
    for (const line of lines) {
      // Issue50: !? matches both visible ## [title] and hidden ## ![title]
...
function generateTOCFromFile(filePath, agendaPath) {
  ...
  // Offset by 1 when a TOC placeholder slide is prepended.
  // 두 가지 prepend 경로 모두 반영:
  //   (a) toc_placeholder 명시(global TOC_PLACEHOLDER || frontmatter fileTocPlaceholder)
  //   (b) _toc 레이아웃 템플릿 존재 시 자동 prepend (Issue46 — generateHTML 1608~1636)
  // (b)는 useTocPlaceholder=false + LAYOUT_TEMPLATES['_toc']가 있을 때 항상 발생
  // (parseMarkdownFile은 isTitle을 useTocPlaceholder=true일 때만 설정하므로
  //  !useTocPlaceholder ⇒ slides[0].isTitle=false ⇒ 자동 prepend 확정).
  const useTocPlaceholder = TOC_PLACEHOLDER || fileTocPlaceholder;
  const autoTocPrepended = !useTocPlaceholder && !!LAYOUT_TEMPLATES['_toc'];
  if (useTocPlaceholder || autoTocPrepended) {
    slideIndex = 1;
  }
...
function generateTocSlideHTML(tocTitle, markmapHtml) {
  if (LAYOUT_TEMPLATES['_toc']) {
    // Issue55: PROJECT_META + downloadButtons 변수 노출 (_toc layout이 헤더에 다운로드 버튼 표시)
    const vars = {
      ...PROJECT_META,
      downloadButtons: PROJECT_DOWNLOADS_HTML,
      title: tocTitle || '',
      content: '',
      markmap: markmapHtml || '',
    };
  ...
  if (isTitleOnly && slide.children && slide.children.length > 0) {
    ...
    const listHtml = `<ul class="chapter-list chapter-list--cards" data-count="${count}">\n` +
      slide.children.map(child =>
        `<li class="chapter-card"><a href="#/${child.index}">${child.title}</a></li>`
      ).join('\n') +
      '\n</ul>';
    html += '\n' + listHtml;
  }
...
function generateSlideHTML(slide) {
  // isTitle/isTable 슬라이드는 layout 적용 대상 아님 (plain으로 위임)
  if (slide.isTitle || slide.isTable) {
    return generatePlainSlideHTML(slide);
  }

  const layoutName = slide.layout || THEME_DEFAULT_LAYOUT;

  if (layoutName && LAYOUT_TEMPLATES[layoutName]) {
    const { content: bodyText, slots } = extractSlots(slide.rawMarkdown || '');
    const title = slide.title || '';
    const contentHtml = convertMarkdownToHTML(bodyText);
    const slotsHtml = {};
    for (const [k, v] of Object.entries(slots)) slotsHtml[k] = convertMarkdownToHTML(v);

    // Issue49: PROJECT_META 값을 layout 변수로 노출 (cover 등에서 사용)
    // Issue55: downloadButtons 변수 노출 (_toc layout에서 사용)
    const vars = { ...PROJECT_META, downloadButtons: PROJECT_DOWNLOADS_HTML, title, content: contentHtml, markmap: '', ...slotsHtml };
    ...
    // Issue27_1: image-only 자동 감지 시 modifier 클래스 추가
    if (slide.autoFullImage) {
      html = html.replace(/<section\b([^>]*)>/, (_, attrs) => {
        if (/class="/.test(attrs)) {
          return `<section${attrs.replace(/class="([^"]*)"/, 'class="$1 layout-blank--full-image"')}>`;
        }
        return `<section${attrs} class="layout-blank--full-image">`;
      });
    }

    // Issue43: video-only 자동 감지 시 modifier 클래스 추가
    if (slide.autoFullVideo) {
      html = html.replace(/<section\b([^>]*)>/, (_, attrs) => {
        if (/class="/.test(attrs)) {
          return `<section${attrs.replace(/class="([^"]*)"/, 'class="$1 layout-blank--full-video"')}>`;
        }
        return `<section${attrs} class="layout-blank--full-video">`;
      });
    }
    return html;
  }
  ...
  if (layoutName && !LAYOUT_TEMPLATES[layoutName]) {
    if (!_WARNED_MISSING_LAYOUTS.has(layoutName)) {
      _WARNED_MISSING_LAYOUTS.add(layoutName);
      const themePaths = THEME_NAME ? `theme/${THEME_NAME}/layouts/ 및 theme/default/layouts/` : 'theme/default/layouts/';
...
function generateHTML(filePath, agendaPath, outputDir, isFirstFile = false, options = {}) {
  ...
  PROJECT_DOWNLOADS_HTML = buildDownloadButtonsHTML(path.dirname(outputDir));
  ...
  // Issue55 Phase 5: single 모드 — isTitle 슬라이드(TOC placeholder) 제거. agenda.html이 그 역할 대체
  // 기존 `toc_placeholder: true` 설정과 무관하게 single 모드는 항상 미생성
  {
    const _hasAgenda = agendaPath && fs.existsSync(agendaPath);
    if (!_hasAgenda && !options.skipTocPlaceholder) {
  ...
  // Step 8: _toc layout 자동 적용
  // _toc 템플릿이 있을 때 isTitle 슬라이드에 layout 설정
  // isTitle 슬라이드는 useTocPlaceholder가 true일 때 자동 prepend됨
  // isTitle이 없어도 _toc 템플릿이 있으면 title 슬라이드 자동 prepend
  // Issue55 Phase 5: single 모드 (no agenda)는 _toc 슬라이드 미생성 — agenda.html이 그 역할 대체
  // Issue55: options.skipTocPlaceholder=true 시 chapter cover-only deck도 _toc 미주입
  if (LAYOUT_TEMPLATES['_toc'] && hasAgenda && !options.skipTocPlaceholder) {
  ...
  // Issue49: cover 슬라이드 자동 주입 (첫 파일 + _config.yml `cover_enabled: true`)
  // 사용자가 이미 _cover layout 슬라이드를 수동 배치했으면 중복 주입 회피
  const coverEnabled = COVER_ENABLED;
  // cover 키 우선 (theme이 먼저 로드되므로 `1.1.cover.html` 같은 theme 전용 템플릿이 alias로 cover에 등록됨)
  // `_cover`는 default fallback
  const coverTpl = LAYOUT_TEMPLATES['cover'] || LAYOUT_TEMPLATES['_cover'];
  if (isFirstFile && coverEnabled && coverTpl) {
    const hasManualCover = slides.some(s => {
      const ln = (s && s.layout) ? String(s.layout).replace(/^_/, '') : '';
      return ln === 'cover';
    });
    if (!hasManualCover) {
      // cover 슬라이드 제목·부제는 options.coverTitle/Subtitle (Issue55) > 첫 슬라이드 frontmatter > 파일명 순
      const head = slides[0] || {};
      slides.unshift({
        title: options.coverTitle || head.title || path.basename(filePath, '.md'),
        subtitle: options.coverSubtitle || head.subtitle || null,
        author: null,
        slogan: null,
        content: '',
        rawMarkdown: '',
        isTitle: false,
        chapterTitle: '',
        layout: 'cover'  // alias 키. theme의 `1.1.cover.html` 또는 default의 `_cover.html` 자동 매핑
      });
    }
  }
  // Issue55 Phase 7: tocOverlaySlideIndex/coverInjected 변수 폐기 — #toc-container 오버레이 제거
  ...
  if (SLIDE_CSS_REL) {
    const cssAbsPath = path.isAbsolute(SLIDE_CSS_REL)
      ? SLIDE_CSS_REL
      : path.join(CONFIG_BASE_DIR, SLIDE_CSS_REL);
...
<body class="${TOP_ALIGN ? 'top-align-mode' : ''} ${GUID_LINE ? 'guide-line-mode' : ''} media-enlarge-${STYLE_CONFIG.style.theContents.media_container_enlarge || 'original'}" style="
  ...
  --title-contents-gap-pct: ${TITLE_CONTENTS_GAP};
">
...
${slidesHTML}
  ...
  <script>
    ...
    // title_contents_gap: title 높이의 % 만큼 margin-bottom을 h2.title에 적용
    function applyTitleContentsGap() {
      var raw = getComputedStyle(document.body).getPropertyValue('--title-contents-gap-pct');
      var pct = parseFloat(raw);
      if (isNaN(pct)) pct = 30;
      document.querySelectorAll('.reveal .slides section h2.title, .reveal .slides section h1.title').forEach(function(el) {
    ...
    document.addEventListener('keydown', function(event) {
      ...
      if (event.key === 'ArrowUp' || event.keyCode === 38) {
        ...
        if (idx.v !== 0) return; // 수직 슬라이드 내부에서는 기본 동작
        if (isCoverSlide(cur)) {
          // Cover 슬라이드: 최상위 — 동작 없음
          event.preventDefault();
          return;
        }
        if (isTocSlide(cur)) {
          // TOC 슬라이드 → agenda.html
          event.preventDefault();
          window.location.href = 'agenda.html';
          return;
        }
        // 본문 슬라이드
        if (M2SLIDE_MODE === 'single') {
          event.preventDefault();
          window.location.href = 'agenda.html';
        } else {
          // Chapter 모드: 같은 deck의 #/toc-placeholder로 이동
          var tocIdx = findTocSlideIndex();
          if (tocIdx >= 0) {
            event.preventDefault();
            Reveal.slide(tocIdx, 0);
          }
        }
        return;
      }

      // ← 키: TOC 슬라이드에서 agenda.html로 이동
      ...
      // → / ↓ 키: Cover 슬라이드 override → agenda.html (D5=B, D6=B)
...
function parseAgenda(agendaPath) {
  ...
  for (const line of lines) {
    ...
    const mainHiddenMatch = line.match(/^## !\[(.+?)\]\((.+?)\)$/);
    if (mainHiddenMatch) {
      currentSection = null; // no TOC entry; subsections also hidden
      continue;
    }
    ...
    // Issue50: ### ![Title](path) — hidden subsection (build, exclude from TOC)
    const subHiddenMatch = line.match(/^### !\[(.+?)\]\((.+?)\)$/);
    if (subHiddenMatch) {
      continue; // skip TOC; navigation functions still include it via !? regex
    }
...
// Issue55 Phase 4: cover_enabled=false 시 index.html → agenda.html 자동 redirect
// D4=A 확정: meta refresh 단독 (JS·앵커 미사용, 정적 HTML 단순화)
...
function generateAgendaHTML({ projectDir, title, tocData, slideCssRel, outputDir }) {
  // 다운로드 자산 검출
  const downloadButtons = buildDownloadButtonsHTML(projectDir);

  // _agenda.html layout 렌더 — 없으면 _toc 폴백, 둘 다 없으면 인라인 fallback
  let bodyHtml;
  const agendaLayoutName = LAYOUT_TEMPLATES['_agenda'] ? '_agenda' : (LAYOUT_TEMPLATES['_toc'] ? '_toc' : null);
  if (agendaLayoutName) {
    ...
    bodyHtml = `<div class="layout-_agenda"><header class="toc-page-header"><h1 class="toc-page-title">${title}</h1><div class="toc-page-downloads">${downloadButtons}</div></header><div class="toc-markmap"><svg class="toc-mindmap-svg"></svg></div></div>`;
  }

  // CSS 링크 산출
  let cssLink = '';
  if (slideCssRel) {
    const cssAbs = path.isAbsolute(slideCssRel) ? slideCssRel : path.join(CONFIG_BASE_DIR, slideCssRel);
  ...
  const expandLevel = STYLE_CONFIG.markmap_depth || 2;
...
${cssLink}
<style>
/* agenda.html 전용 최소 보정 — 슬라이드가 아니라 문서이므로 reveal.js 컨텍스트 없음 */
...
.layout-_toc { padding: 24px; box-sizing: border-box; }
.toc-mindmap-svg { width: 100%; height: calc(100vh - 200px); display: block; }
.toc-markmap { padding: 12px 0; }
</style>
</head>
<body>
${bodyHtml}
...
// Issue55 Phase 9: ↑ 키로 Cover Page(index.html)로, → / ↓ 키로 다음 페이지(첫 챕터 또는 deck 본문)로 이동
(function(){
  const _tocData = ${tocDataJson};
  // tocData.children[0]의 content에서 첫 href를 추출 — chapter 모드면 첫 챕터 HTML, single 모드면 index.html#/N
  ...
  document.addEventListener('keydown', function(e){
    if (e.key === 'ArrowUp' || e.keyCode === 38 || e.key === 'ArrowLeft' || e.keyCode === 37) {
      e.preventDefault();
      window.location.href = 'index.html';
    } else if (e.key === 'ArrowRight' || e.keyCode === 39 || e.key === 'ArrowDown' || e.keyCode === 40 || e.key === 'PageDown' || e.keyCode === 34 || e.key === ' ' || e.keyCode === 32) {
      if (nextUrl) {
        e.preventDefault();
...
function main() {
  ...
  // Quick-parse projectDir so loadConfig can find project-level _config.yml
  if (args.length === 0) {
  ...
  // Load config now that projectDir is known
  loadConfig(projectDir);

  // Issue48: 프로젝트 운영 메타 로드 (_meta.yml)
  loadProjectMeta(projectDir);
  ...
  // Issue26: video 폴더 복사 (mp4 등 동영상 자산 지원)
  const videoInputDir = path.join(inputDir, 'video');
  const videoOutputDir = path.join(outputDir, 'video');
  if (fs.existsSync(videoInputDir)) {
    console.log(`\nCopying videos from ${videoInputDir} to ${videoOutputDir}`);
    if (fs.existsSync(videoOutputDir)) {
      fs.rmSync(videoOutputDir, { recursive: true, force: true });
    }
    fs.cpSync(videoInputDir, videoOutputDir, { recursive: true });
    console.log('✅ Videos copied successfully');
  }

  // Theme img 폴더 복사 (theme/{name}/img/ → slide/theme-img/) — 마스코트·장식 이미지
  const themeNameForImg = THEME_NAME || 'default';
  const themeImgInputDir = path.join(ROOT_DIR, 'theme', themeNameForImg, 'img');
  const themeImgOutputDir = path.join(outputDir, 'theme-img');
  if (fs.existsSync(themeImgInputDir)) {
    console.log(`\nCopying theme images from ${themeImgInputDir} to ${themeImgOutputDir}`);
    if (fs.existsSync(themeImgOutputDir)) {
      fs.rmSync(themeImgOutputDir, { recursive: true, force: true });
    }
    fs.cpSync(themeImgInputDir, themeImgOutputDir, { recursive: true });
    console.log('✅ Theme images copied successfully');
  }

  // Copy Custom CSS if defined
  if (SLIDE_CSS_REL) {
    const cssAbsPath = path.isAbsolute(SLIDE_CSS_REL) ? SLIDE_CSS_REL : path.join(CONFIG_BASE_DIR, SLIDE_CSS_REL);
  ...
  // Issue55 Phase 4·5·8: 모드 통일 출력 흐름
  // Single 모드 (hasAgenda=false): index.html = 본문 deck (cover + 본문, _toc 미포함)
  // Chapter 모드 (hasAgenda=true): 0X-*.html = 챕터 deck (TOC + 본문) + index.html = cover-only lightweight deck
  // 두 모드 공통: agenda.html = standalone 마크맵 페이지

  filesToProcess.forEach((file, idx) => {
    const inputPath = path.join(inputDir, file);
    const isFirstFile = idx === 0;
    // Single 모드: 출력 파일명을 index.html로 통일 (D1=A 확정)
    // Chapter 모드: 챕터 파일명 유지 (0X-*.html)
    const outputName = (!hasAgenda) ? 'index.html' : file.replace('.md', '.html');
    const outputPath = path.join(outputDir, outputName);

    console.log(`Processing: ${file}`);
    // Issue49: 첫 파일에만 cover 주입 신호 전달
    // Chapter 모드의 챕터 deck (0X-*.html)은 cover 미주입 — cover는 index.html이 담당
    const isCoverFile = !hasAgenda && isFirstFile;
    const html = generateHTML(inputPath, hasAgenda ? agendaPath : null, outputDir, isCoverFile);
    fs.writeFileSync(outputPath, html, 'utf-8');
    console.log(`  → Generated: ${outputPath}`);
  });

  // Issue55 Phase 3: agenda.html (standalone) — 두 모드 공통 생성
  console.log('\nGenerating agenda.html...');
  const firstMdPath = path.join(inputDir, filesToProcess[0]);
  let agendaTitle = '';
  let agendaTocData;
  if (hasAgenda) {
    // Chapter 모드: AGENDA.md 파싱 결과 사용
    ...
    // Single 모드: 첫 마크다운 파일의 슬라이드 H1/H2 트리 사용
    agendaTocData = generateTOCFromFile(firstMdPath, null);
    // 링크는 #/N 형식 — agenda.html에서 클릭 시 index.html#/N로 이동해야 하므로 rewrite
    ...
    // 제목: 첫 md 파일의 frontmatter title 우선
    const fileRaw = fs.readFileSync(firstMdPath, 'utf-8');
    const fmMatch = fileRaw.match(/^---[\s\S]*?^title:\s*(.+)$/m);
    if (fmMatch) {
      agendaTitle = fmMatch[1].trim();
    } else {
      const h1Match = fileRaw.match(/^# (.+)$/m);
      agendaTitle = h1Match ? h1Match[1].trim() : path.basename(firstMdPath, '.md');
    }
  }
  // PROJECT_DOWNLOADS_HTML 갱신 (마지막 generateHTML 호출 후 stale 가능성 방지)
  PROJECT_DOWNLOADS_HTML = buildDownloadButtonsHTML(projectDir);
  const agendaHTML = generateAgendaHTML({
    projectDir, title: agendaTitle, tocData: agendaTocData,
    slideCssRel: SLIDE_CSS_REL, outputDir
  });
  const agendaPath2 = path.join(outputDir, 'agenda.html');
  fs.writeFileSync(agendaPath2, agendaHTML, 'utf-8');
  console.log(`✅ Generated: ${agendaPath2}`);

  // Issue55 Phase 4: index.html 분기 출력
  const indexPath = path.join(outputDir, 'index.html');
  if (!COVER_ENABLED) {
    // cover_enabled=false: index.html을 redirect-only HTML로 덮어쓰기 (D4=A)
    ...
    // Chapter 모드 + cover_enabled=true: cover-only lightweight deck (D2=B 확정)
    // 임시 markdown 파일에 frontmatter만 작성 → generateHTML 호출 → 임시 파일 삭제
    console.log('\nGenerating index.html (cover-only lightweight deck)...');
    const tmpMdPath = path.join(outputDir, '__cover_only__.md');
    const tmpMdContent = `---\ntitle: ${agendaTitle}\n---\n`;
    fs.writeFileSync(tmpMdPath, tmpMdContent, 'utf-8');
    try {
      const indexHTML = generateHTML(tmpMdPath, null, outputDir, true, {
        skipTocPlaceholder: true,
        coverTitle: agendaTitle,
      });
  ...
  // Single 모드 + cover_enabled=true: index.html은 위 forEach에서 이미 생성됨
