...
let PROJECT_META = {};          // Issue48: 프로젝트 운영 메타 (meta.yml — instructor, version, lecture_date 등)
let STYLE_CONFIG = {
...
// _doc_design/meta-yml.md 스키마 참조. 미존재 시 silent skip (backward compatible).
function loadProjectMeta(projectDir) {
  if (!projectDir) return;
  const metaPath = path.join(projectDir, 'meta.yml');
  ...
  try {
    ...
    console.log(`✅ meta.yml loaded: ${metaPath} (${keys.length} fields: ${keys.slice(0, 4).join(', ')}${keys.length > 4 ? '...' : ''})`);
  } catch (e) {
    console.warn(`⚠️ meta.yml parse failed: ${metaPath} — ${e.message}`);
    PROJECT_META = {};
  }
}
...
function generateSlideHTML(slide) {
  ...
  if (layoutName && LAYOUT_TEMPLATES[layoutName]) {
    ...
    const vars = { ...PROJECT_META, title, content: contentHtml, markmap: '', ...slotsHtml };
...
function generateHTML(filePath, agendaPath, outputDir, isFirstFile = false) {
  ...
  // Issue49: cover 슬라이드 자동 주입 (첫 파일 + meta.yml `cover_enabled: true`)
  // 사용자가 이미 _cover layout 슬라이드를 수동 배치했으면 중복 주입 회피
  const coverEnabled = String(PROJECT_META.cover_enabled || '').toLowerCase() === 'true';
  const coverTpl = LAYOUT_TEMPLATES['_cover'] || LAYOUT_TEMPLATES['cover'];
  if (isFirstFile && coverEnabled && coverTpl) {
    ...
    if (!hasManualCover) {
      // cover 슬라이드 제목·부제는 첫 슬라이드(isTitle 포함)의 frontmatter title/subtitle 우선
      const head = slides[0] || {};
      slides.unshift({
        title: head.title || path.basename(filePath, '.md'),
...
<head>
  ...
  <style>
    ...
    #toc-container .title-card {
      flex-shrink: 0;
      text-align: var(--main-title-align, center);
    }
    #toc-container p.main-subtitle {
      font-family: var(--main-title-font-family, var(--global-font-family, inherit));
      color: var(--main-title-color, #555);
      font-size: calc(var(--main-title-font-size, 80px) * 0.5);
      font-weight: 400;
      margin: 0;
      padding: 4px 0 8px 0;
      text-align: var(--main-title-align, center);
    ...
    #toc-container p.main-author {
      font-family: var(--main-title-font-family, var(--global-font-family, inherit));
      color: var(--main-title-color, #555);
      font-size: calc(var(--main-title-font-size, 80px) * 0.35);
      font-weight: 400;
      margin: 0;
      padding: 4px 0 4px 0;
      text-align: var(--main-title-align, center);
    ...
    #toc-container p.main-slogan {
      font-family: var(--main-title-font-family, var(--global-font-family, inherit));
      color: var(--main-title-color, #888);
      font-size: calc(var(--main-title-font-size, 80px) * 0.3);
      font-weight: 300;
      font-style: italic;
      margin: 0;
...
">
  ...
  <div id="toc-container">
    <div class="title-card">
      <h1 class="main-title">${escapeHtmlAttr(titleMeta.title || title)}</h1>
      ${titleMeta.subtitle ? `<p class="main-subtitle">${escapeHtmlAttr(titleMeta.subtitle)}</p>` : ''}
      ${titleMeta.author ? `<p class="main-author">${escapeHtmlAttr(titleMeta.author)}</p>` : ''}
      ${titleMeta.slogan ? `<p class="main-slogan">${escapeHtmlAttr(titleMeta.slogan)}</p>` : ''}
    </div>
...
function main() {
  ...
  // Issue48: 프로젝트 운영 메타 로드 (meta.yml)
  ...
  filesToProcess.forEach((file, idx) => {
    ...
    const html = generateHTML(inputPath, hasAgenda ? agendaPath : null, outputDir, isFirstFile);
