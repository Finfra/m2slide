...
// Read minimal config (YAML subset).
// Search order: projectDir/_config.yml → ROOT_DIR/_config.yml → ROOT_DIR/_config.org.yml
function loadConfig(projectDir) {
  try {
    const candidates = [];
    if (projectDir) candidates.push({ p: path.join(projectDir, '_config.yml'), base: projectDir });
    candidates.push({ p: path.join(ROOT_DIR, '_config.yml'), base: ROOT_DIR });
    candidates.push({ p: path.join(ROOT_DIR, '_config.org.yml'), base: ROOT_DIR });

    let cfgPath = null;
    for (const c of candidates) {
      if (fs.existsSync(c.p)) { cfgPath = c.p; CONFIG_BASE_DIR = c.base; break; }
    }
    if (!cfgPath) return;
...
function parseMarkdownFile(filePath) {
  ...
  const slideObjects = slides.map((slide, index) => {
    ...
    // First slide gets title + frontmatter metadata
    if (index === 0) {
      let title = yamlTitle;
      if (!title) {
        title = currentChapterTitle || 'Slide';
      }
      return { title, subtitle: yamlSubtitle, author: yamlAuthor, slogan: yamlSlogan, content: slide, isTitle: true, chapterTitle: '' }; // No chapter header on TOC
    }
