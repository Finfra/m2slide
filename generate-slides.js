...
function processInline(text) {
  ...
  // Images
  text = text.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, '<img src="../md/$2" alt="$1">');
...
function getSubsections(fileName) {
  try {
    const agendaPath = path.join(__dirname, 'AGENDA.md');
    const content = fs.readFileSync(agendaPath, 'utf-8');
    const lines = content.split('\n');

    // Find the main section for this file
    const mainPattern = new RegExp(`## \\[(.+?)\\]\\(md/${fileName}\\)`);
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (foundMainSection) {
        ...
        if (subMatch) {
          ...
          const htmlFile = mdPath.replace('md/', '').replace('.md', '.html');
...
function getParentPage(fileName) {
  try {
    ...
    const subPattern = new RegExp(`### \\[(.+?)\\]\\(md/${fileName}\\)`);
      ...
      if (subPattern.test(line)) {
        ...
        for (let j = i - 1; j >= 0; j--) {
          ...
          if (parentMatch) {
            ...
            return mdPath.replace('md/', '').replace('.md', '.html');
          }
        }
      }
    }
...
function parseAgenda(agendaPath) {
  ...
  for (const line of lines) {
    ...
    if (mainMatch) {
      ...
      const htmlPath = mdPath.replace('md/', '').replace('.md', '.html');
...
function main() {
  ...
  if (args.length === 0) {
    ...
    files.forEach(file => {
      ...
      const outputPath = path.join(__dirname, 'slide', file.replace('.md', '.html'));
    ...
    const indexPath = path.join(__dirname, 'slide', 'index.html');
    ...
    const outputPath = path.join(__dirname, 'slide', `${baseName}.html`);
